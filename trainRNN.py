def trainNet():
    ...
    input = np.row_stack([filterIn,tmp2])
    ##
    
    #net  = uf.rnnTrainAUX(input)
    ##
    ...
    ds.newSequence()
    for idx1 in range(s):
        ds.appendLinked(input[idx1],(1,0,0))

    ds.newSequence()
    for idx2 in range(s):
        ds.appendLinked(input[idx2+29],(0,1,0))

    ds.newSequence()
    for idx3 in range(s):
        ds.appendLinked(input[idx3+29+29],(0,0,1))
